...
# commented out in order to initialise it from Makefile.local
# BASE_CFLAGS =
...
#############################################################################
# SETUP AND BUILD -- Android (as a part of linux built)
#############################################################################
  #ifeq ($(COMPILE_PLATFORM),android)
  ifeq ($(COMPILE_PLATFORM),android)
    # we need the app to be store in a shared library
    CLIENT_LDFLAGS += -fpic -shared 
    
    # for __android_log_print(msg)
    CLIENT_LDFLAGS += -llog 
    
    # for android/looper.h/ALooper_pollAll() & ANativeWindow_getWidth ...
    CLIENT_LDFLAGS += -landroid 

    # for Android Direct Sound
    CLIENT_LDFLAGS += -lOpenSLES 

    # for native_app_glue
    BASE_CFLAGS += -I$(ANDIR)/native_app_glue -u ANativeActivity_onCreate

    # already used in SDL, so I continue with it in net_ip & vk.c...
    BASE_CFLAGS += -D__ANDROID__ 
    
    # create app lib name "libquake3e.so"
    ARCHEXT = lib
    BINEXT = .so
  endif
#############################################################################

# those 3 lines are common to *NIX & Android
  DEBUG_CFLAGS = $(BASE_CFLAGS) -DDEBUG -D_DEBUG -g -O0 
  RELEASE_CFLAGS = $(BASE_CFLAGS) -DNDEBUG $(OPTIMIZE)
...
TARGET_CLIENT = $(ARCHEXT)$(CNAME)$(BINEXT)
...
ifneq ($(COMPILE_PLATFORM),android)
    Q3OBJ += \
...
ifeq ($(COMPILE_PLATFORM),android)
    Q3OBJ += \
        $(B)/client/android_VKimp.o \
        $(B)/client/android_snd.o

ifeq ($(USE_VULKAN_API),1)
    Q3OBJ += \
        $(B)/client/android_qvk.o
endif
