...
#include <pthread.h>
qboolean storagePermissionsGranted;
#endif
...
// FIXME TTimo need to actual the cursor if needed??
void tty_Show( void )
{
	if ( !ttycon_on )
		return;
...
void Sys_Exit( int code )
{
	Sys_ConsoleInputShutdown();
#if defined(__ANDROID__)
    ANativeActivity_finish(AppState.NativeApp->activity);
#endif
...
#else
	fprintf( stdout, "Android Quake3 Executable  [%s %s]\n", date, time );
#endif
...
//	memset( &sys_packetReceived[0], 0, sizeof( sys_packetReceived ) );
	...
	while (1)
#else
	while ( AppState.NativeApp->destroyRequested == 0 )
#endif
	{
		// Render quake3 frame.
#ifdef __linux__
...
#else
#ifdef USE_VRAPI
		// TODO https://developer.oculus.com/blog/bringing-phase-sync-to-mobile-vr/?locale=fr_FR
	...
	// never gets here (except android application exiting)
	return 0;
}

/*
================================================================================
Android Native Activity specific
================================================================================
*/
#if defined( __ANDROID__ )
/*
...
{
	androidApp_struct * _appState = (androidApp_struct *)app->userData;

	switch ( cmd )
	{
		case APP_CMD_GAINED_FOCUS:
		{
			_appState->Focused = qtrue;

			//GUNNM TODO call recenter?
			cl.viewangles[PITCH] = 0;//SHORT2ANGLE(cl.snap.ps.delta_angles[PITCH]);//GUNNM TODO
			VectorCopy(cl.viewangles, vr_info.rightAngles);
			break;
		}
		...
		case APP_CMD_LOST_FOCUS:
			_appState->Focused = qfalse;
			break;
		case APP_CMD_RESUME:
		{
			_appState->Resumed = qtrue;
			break;
		}
		case APP_CMD_PAUSE:
		{
			_appState->Resumed = qfalse;
			break;
		}
		case APP_CMD_STOP:
		{
		...
		{
			_appState->NativeWindow = NULL;
			break;
		}
		case APP_CMD_INIT_WINDOW:
		{
			_appState->NativeWindow = app->window;
			_appState->nativeWindow_Width = ANativeWindow_getWidth(app->window); // oculus GO => 2560, this is the full LCD size
			_appState->nativeWindow_Height = ANativeWindow_getHeight(app->window);
			break;
		}
...
/*
================================================================================
JNI StoragePermissionsGranted
================================================================================
*/
EXPORT_API void Java_com_GUNNM_1VR_Quake3eVR_MainActivity_StoragePermissionsGranted( JNIEnv *jni, jclass clazz, int permissionsGranted )
{
...
================================================================================
*/
void App_Clear(androidApp_struct * app )
{
#ifdef USE_VRAPI
	app->Java.Vm = NULL;
	app->Java.Env = NULL;
	...
	app->BackButtonDownLastFrame = qfalse;
}
...
void android_main( struct android_app* app ) {
	...
	app->onAppCmd = app_handle_cmd; // Set the callback to process system events
    AppState.NativeApp = app;

	// Wait storage permissions and app.window initialisation
	while (AppState.NativeWindow == NULL || !storagePermissionsGranted) {
		if (ALooper_pollAll( 0, NULL, &events, (void**)&source) >= 0) {
			if (source != NULL) source->process(app, source);
		}
	}

	// Launch game with command line parameters for debug use
	char *argv[4];
	int argc=0;
	argv[argc] = strdup("dummy arg"); // fixme: the first arg is not read
	argc++;
	//argv[argc] = strdup("+set cg_drawfps 1");
	//argc++;
	argv[argc] = strdup("+set fs_homepath \"sdcard/quake3\"");
	argc++;

	// Launch the game
	main( argc, (const char**) argv );

	//GUNNM TODO clear ovrapp, etc.
