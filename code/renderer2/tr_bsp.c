...
void RE_LoadWorldMap( const char *name ) {
	...
	if (0)
	{
		...
		{
			//int i;
			byte *buffer2 = ri.Malloc(w->lightGridBounds[0] * w->lightGridBounds[1] * 3 + 18);
			byte *out;
			uint8_t *in;
			char fileName[MAX_QPATH];
			
			Com_Memset (buffer2, 0, 18);
			buffer2[2] = 2;		// uncompressed type
			buffer2[12] = w->lightGridBounds[0] & 255;
			buffer2[13] = w->lightGridBounds[0] >> 8;
			buffer2[14] = w->lightGridBounds[1] & 255;
			buffer2[15] = w->lightGridBounds[1] >> 8;
			buffer2[16] = 24;	// pixel size
			...
			for (i = 0; i < w->lightGridBounds[2]; i++)
			{
				...
				ri.FS_WriteFile(fileName, buffer2, w->lightGridBounds[0] * w->lightGridBounds[1] * 3 + 18);
			}
